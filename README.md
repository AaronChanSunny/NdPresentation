...
`Android` 的 `UI` 控件并不是线程安全的，也就说只能在 `UI` 线程也就说主线程更新 `UI`，如果在非 `UI` 线程更新 `UI`，则可能导致异常。另外一方面，`Android` 又要求不能在 `UI` 线程执行耗时的任务。然而，我们在开发 `App` 过程中，经常面对的场景是，处理一段耗时的逻辑，处理完成后将结果呈现给 `UI`，这就产生了矛盾，因此 `Android` 弄出了一套消息机制，专门用来处理任务在主线程和工作线程之间的切换，并且在这套消息机制上封装出了诸如 `HandlerThread`、`AsyncTask`、`IntentService`等异步类。开发者可以在工作线程进行耗时的操作，待耗时操作完成后，通过异步框架将线程切换到 `UI` 线程，呈现操作结果。
